...
# 2020-07-30
# 
# Train pretrain tokonly unalign
#
...
CKPT_PATH=/expscratch/detter/vismt/zh/20200730/unaligned/chars/checkpoints/model_ckpt_best.pth
...
python -u ${SRC_PATH}/fairseq-ocr/train_unalign.py \
${DATA_DIR} \
--user-dir ${SRC_PATH} \
--save-dir ${EXP_DIR}/${TYPE} \
--arch 'vis_unalign_transformer_iwslt_de_en' \
--image-pretrain-path $TMPDIR/vismt \
--image-samples-path ${EXP_DIR}/${TYPE}/samples \
--image-checkpoint-path ${CKPT_PATH} \
--image-embed-type 'tokonly' \
--image-pretrain-eval-only \
--source-lang 'zh' \
--target-lang 'en' \
--left-pad-source 0 \
--left-pad-target 0 \
--task 'visunaligntranslation' \
--train-subset 'train' \
--valid-subset 'valid' \
--criterion 'vis_unalign_label_smoothed_cross_entropy' \
--adam-betas '(0.9, 0.98)' \
--adam-eps 1e-08 \
