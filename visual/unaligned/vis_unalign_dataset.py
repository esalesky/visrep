...
def image_collater(batch):
    # width is either token len or sentence image width
    ...
    largest_image = batch[0][0].size()
    input_tensor = torch.ones(
        len(batch), largest_image[0], largest_image[1], largest_image[2])
    for idx, (tensor, transcript, metadata) in enumerate(batch):
        width = tensor.size(2)
    ...
    LOG.debug('COLLATE: target_length %s %s %s', len(
        batch['target_length']), batch['target_length'].sum(), batch['target_length'])
    LOG.debug('COLLATE: target %s %s',
              len(batch['target']), batch['target'])
    ...
    def __getitem__(self, idx):
        ...
        if h != self.image_height:
            cv_image = self.image_resize(
                cv_image, height=self.image_height)
        (h, w) = cv_image.shape[:2]
        line_image = self.transform(cv_image)
        out_width = w
        ...
        LOG.debug('GETITEM: image shape %s', line_image.shape)
