...
def train(args, trainer, task, epoch_itr):
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        if type(samples[0]["net_input"]["src_images"]) != type(None):
            LOG.debug(
                "DATA: src_images %s", samples[0]["net_input"]["src_images"].shape
            )
        ...
        #         # tokens_list = samples[img_idx]["net_input"]["src_tokens"].cpu().numpy()
        #         tokens_list = samples[img_idx]["target"].cpu().numpy()
        #         image_list = samples[img_idx]["net_input"]["src_images"].cpu().numpy()
        ...
        #         )
        #         hyp_transcriptions = decode(task.src_dict, logits, input_lengths)
        ...
        #             image = np.uint8(image_list[token_idx].transpose((1, 2, 0)) * 255)
        #             # rgb to bgr
        #             image = image[:, :, ::-1].copy()
        #             curr_out_path = os.path.join(
        #                 samples_train_output,
        #                 str(i) + "_" + str(img_idx) + "_" + str(token_idx) + ".png",
        #             )
        #             cv2.imwrite(curr_out_path, image)

        #             task.tensorboard_writer.add_image(
        #                 "images/valid/{}_{}_{}".format(
        #                     str(i), str(img_idx), str(token_idx)
        #                 ),
        #                 F.to_tensor(image),
        #                 0,
        #             )
        ...
        #     "train/train_loss",
        #     stats["loss"].avg,
        #     (epoch_itr.epoch - 1) * len(epoch_itr) + epoch_itr.iterations_in_epoch,
        # )
...
def validate(args, trainer, task, epoch_itr, subsets):
    ...
    samples_valid_output = os.path.join(task.args.image_samples_path, "valid")
    ...
    for subset in subsets:
        ...
        for sample in progress:
            ...
            image_total += int(sample["net_input"]["src_tokens"].size(0)) * int(
            ...
            for hyp_ctr in range(len(hyp_transcriptions)):
                ...
                if valid_batch_ctr % 10 == 0 and hyp_ctr < 2:
                    ...
                    image = np.uint8(image_list[hyp_ctr].transpose((1, 2, 0)) * 255)
                    # rgb to bgr
                    image = image[:, :, ::-1].copy()
                    curr_out_path = os.path.join(
                        samples_valid_output,
                        str(valid_batch_ctr) + "_" + str(hyp_ctr) + ".png",
                    )
                    cv2.imwrite(curr_out_path, image)

                    task.tensorboard_writer.add_image(
                        "images/valid/{}_{}".format(str(valid_batch_ctr), str(hyp_ctr)),
                        F.to_tensor(image),
                        0,
                    )
        ...
        LOG.info(
            "Valid images %d, cer %.2f, wer %.2f",
            batch_size_total,
            total_ref_char_dist / total_ref_chars,
            total_ref_word_dist / total_ref_words,
        )
