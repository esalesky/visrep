...
class CTCLossCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduction='mean'):
        ...
        loss, _ = self.compute_loss(model, net_output, sample, reduction=reduction)
    ...
    def compute_loss(
        ...
        targets = torch.cat(sample['target']).cpu() 
        target_lengths = sample['target_length'] 
        input_lengths = torch.full((sample['nsentences'],), log_probs.size(0), dtype=torch.int32).cuda()
