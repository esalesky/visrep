...
@register_criterion('vis_unalign_label_smoothed_cross_entropy')
class VisUnAlignLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_src_loss(self, model, net_output, sample, reduce=True):
        ...
        # on src loss, src tokens are the targets
        src_targets = sample['net_input']['src_tokens'].view(-1)
        src_target_lengths = sample['net_input']['src_lengths'].view(-1)

        pad_mask = src_targets != self.pad_idx
        src_targets_flat = src_targets.masked_select(pad_mask)

        LOG.debug('CRITERION: batch size %s', batch_size)
        LOG.debug('CRITERION: pad index %s', self.pad_idx)
        LOG.debug('CRITERION: targets %s', src_targets)
        LOG.debug('CRITERION: targets %s', src_targets.shape)

        LOG.debug('CRITERION: target_lengths %s', src_target_lengths.shape)
        LOG.debug('CRITERION: target_lengths %s', src_target_lengths)
        LOG.debug('CRITERION: target_lengths %s', src_target_lengths.sum())

        LOG.debug('CRITERION: logits %s', logits.shape)
        LOG.debug('CRITERION: log_probs %s', log_probs.shape)
        LOG.debug('CRITERION: input_lengths %s', input_lengths.shape)

        # CUDA, PyTorch native implementation: OK
        torch.backends.cudnn.enabled = False
        loss = F.ctc_loss(log_probs.to('cuda'),
                          src_targets_flat.to('cuda'),
                          input_lengths,
                          src_target_lengths,
                          reduction='sum',
                          zero_infinity=True)
