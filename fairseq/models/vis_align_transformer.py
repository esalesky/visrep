...
from fairseq.modules.vis_align_ocr import (
    AlignOcrModel,  # AlignOCR, vista_ocr,
)
...
@register_model('vis_align_transformer')
class VisAlignTransformerModel(FairseqEncoderDecoderModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return VisAlignTransformerEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return VisAlignTransformerDecoder(
            args,
            tgt_dict,
            embed_tokens,
...
@register_model('vis_align_transformer_align')
class VisAlignTransformerAlignModel(VisAlignTransformerModel):
    """
    ...
    def add_args(parser):
        # fmt: off
        super(VisAlignTransformerAlignModel,
              VisAlignTransformerAlignModel).add_args(parser)
    ...
    def build_model(cls, args, task):
        ...
        transformer_model = VisAlignTransformerModel.build_model(args, task)
        return vis_align_TransformerAlignModel(transformer_model.encoder, transformer_model.decoder, args)
...
class VisAlignTransformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        model = AlignOcrModel(args, dictionary,
        ...
        if args.image_checkpoint_path:
            ...
            LOG.info('Loading AlignOcrModel checkpoint...%s',
        ...
        if self.args.image_pretrain_eval_only:
            LOG.info('...setting requires_grad to False for AlignOcrModel')
    ...
    def forward(self, src_tokens, src_lengths, src_images, cls_input=None, return_all_hiddens=False):
        ...
        if self.args.image_embed_type == 'visonly':
            ...
            x = (x_tok + x_vis) / 2
            encoder_embedding = (encoder_embedding_tok +
                                 encoder_embedding_vis) / 2
            if self.args.image_verbose:
                LOG.debug('ENCODER: avg x  %s', x.shape)
                LOG.debug('ENCODER: avg encoder_embedding  %s',
                          encoder_embedding.shape)
        elif self.args.image_embed_type == 'add':
            x = x_tok + x_vis
            encoder_embedding = encoder_embedding_tok + encoder_embedding_vis
            if self.args.image_verbose:
                LOG.debug('ENCODER: add x %s', x.shape)
                LOG.debug('ENCODER: add encoder_embedding %s',
                          encoder_embedding.shape)
        elif self.args.image_embed_type == 'concat':
            x = torch.cat((x_tok, x_vis), dim=2)
            LOG.debug('ENCODER: concat %s', x.shape)
            x = self.vis_concat_x_linear(x)
            LOG.debug('ENCODER: concat linear %s', x.shape)
            encoder_embedding = torch.cat(
                (encoder_embedding_tok, encoder_embedding_vis), dim=2)
            encoder_embedding = self.vis_concat_embed_linear(encoder_embedding)
            if self.args.image_verbose:
                LOG.debug('ENCODER: concat x %s', x.shape)
                LOG.debug('ENCODER: concat encoder_embedding %s',
                          encoder_embedding.shape)
        else:
            LOG.debug('EXCEPTION: unkown image_embed_type %s',
...
class VisAlignTransformerDecoder(FairseqIncrementalDecoder):
    """
...
@register_model_architecture('vis_align_transformer', 'vis_align_transformer')
...
@register_model_architecture('vis_align_transformer', 'vis_align_transformer_iwslt_de_en')
