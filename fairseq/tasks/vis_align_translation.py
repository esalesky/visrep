...
from fairseq.vis_align_sequence_generator import (
    VisAlignSequenceGenerator,
    VisAlignSequenceGeneratorWithAlignment
)

from fairseq.data.vis_align_indexed_dataset import (
    VisAlignIndexedRawTextDataset,
    make_dataset,
    dataset_exists
)

from fairseq.data.vis_align_language_pair_dataset import (
    VisAlignLanguagePairDataset
)
...
def load_langpair_dataset(
    ...
    for k in itertools.count():
        ...
        if args.image_cache_path:
            if not os.path.exists(args.image_cache_path):
                os.makedirs(args.image_cache_path)

        LOG.info('prefix %s src %s', prefix, src)
        img_dataset = VisAlignIndexedRawTextDataset(
            split=split,
            path=prefix + src,
            dictionary=src_dict,
            ...
            cache_output=args.image_cache_path,
    ...
    return VisAlignLanguagePairDataset(
...
@register_task('visaligntranslation')
class VisAlignTranslationTask(FairseqTask):
    """
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths):
        return VisAlignLanguagePairDataset(src_tokens, src_lengths, self.source_dictionary)
    ...
    def build_generator(self, args):
        if getattr(args, 'score_reference', False):
            ...
            from fairseq.vis_align_sequence_generator import VisAlignSequenceGenerator, VisAlignSequenceGeneratorWithAlignment
            if getattr(args, 'print_alignment', False):
                seq_gen_cls = VisAlignSequenceGeneratorWithAlignment
            else:
                seq_gen_cls = VisAlignSequenceGenerator
            return seq_gen_cls(
