...
def infer_dataset_impl(path):
    if VisUnAlignIndexedRawTextDataset.exists(path):
        return 'raw'
    elif VisUnAlignIndexedDataset.exists(path):
        with open(index_file_path(path), 'rb') as f:
            magic = f.read(8)
            if magic == VisUnAlignIndexedDataset._HDR_MAGIC:
                return 'cached'
            elif magic == VisUnAlignMMapIndexedDataset.Index._HDR_MAGIC[:8]:
                return 'mmap'
            else:
                return None
    else:
...
def make_builder(out_file, impl, vocab_size=None):
    if impl == 'mmap':
        return VisUnAlignMMapIndexedDatasetBuilder(out_file, dtype=__best_fitting_dtype(vocab_size))
    else:
        return VisUnAlignIndexedDatasetBuilder(out_file)


def make_dataset(split, path, impl, fix_lua_indexing=False, dictionary=None):
    if impl == 'raw' and VisUnAlignIndexedRawTextDataset.exists(path):
        assert dictionary is not None
        return VisUnAlignIndexedRawTextDataset(split, path, dictionary)
    elif impl == 'lazy' and VisUnAlignIndexedDataset.exists(path):
        return VisUnAlignIndexedDataset(path, fix_lua_indexing=fix_lua_indexing)
    elif impl == 'cached' and VisUnAlignIndexedDataset.exists(path):
        return VisUnAlignIndexedCachedDataset(path, fix_lua_indexing=fix_lua_indexing)
    elif impl == 'mmap' and VisUnAlignMMapIndexedDataset.exists(path):
        return VisUnAlignMMapIndexedDataset(path)
    return None


def dataset_exists(path, impl):
    if impl == 'raw':
        return VisUnAlignIndexedRawTextDataset.exists(path)
    elif impl == 'mmap':
        return VisUnAlignMMapIndexedDataset.exists(path)
    else:
        return VisUnAlignIndexedDataset.exists(path)
...
class VisUnAlignIndexedDataset(FairseqDataset):
...
class VisUnAlignIndexedCachedDataset(VisUnAlignIndexedDataset):
...
class VisUnAlignIndexedRawTextDataset(FairseqDataset):
    ...
    def __init__(self,
        ...
        LOG.info('...VisUnAlignIndexedRawTextDataset %s', path)
        self.tokens_list = []
        self.lines = []
        self.sizes = []
    ...
    def __getitem__(self, i):
        ...
        if self.image_pretrain_path:
            ...
            img_channel, img_height, img_width = decode_metadata['image'].shape
            curr_img = decode_metadata['image']
        else:
            ...
            cv_image = self.image_pad(
                cv_image, input_pad_height=self.image_height)
            (h, w) = cv_image.shape[:2]
            # sometimes pad height is off by a pixel
            if h != self.image_height:
                cv_image = self.image_resize(
                    cv_image, height=self.image_height)
            curr_img = cv_image
        ...
        line_image = []  # use list to be consist with align dataset
...
class VisUnAlignIndexedDatasetBuilder(object):
    element_sizes = {
        np.uint8: 1,
        np.int8: 1,
        np.int16: 2,
...
class VisUnAlignMMapIndexedDataset(torch.utils.data.Dataset):
    class Index(object):
...
class VisUnAlignMMapIndexedDatasetBuilder(object):
