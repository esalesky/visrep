...
class VisualTextRegDataset(LanguagePairDataset):
    """
    ...
    def __getitem__(
        ...
        assert VisualTextRegDataset.image_generator is not None, "UNEXPECTED: RAW DATA BUT CLASS IMAGE_GENERATOR IS NONE"
        if VisualTextRegDataset.image_generator is not None:
            # window regularization -- regenerate slices, update source size given new src len
            window = random.choice([15, 20, 25, 30])

            # not necessary to update source_images (1d pixel stream, flattened image_tensor)
            image_tensor = VisualTextRegDataset.image_generator.get_tensors(self.src_text[index], window)
...
#            print(example["source"].shape)
            ...
            # this is stored as a 1d pixel stream, so reshape (harmless for raw text)
            if self.slice_area:
    ...
    def from_raw(
            ...
            epoch: int,
            seed: int,
    ) -> "VisualTextRegDataset":
        """
        Builds a VisualTextRegDataset from an input path and some command-line parameters.

        There are two ways to build: from plain text, or from a preprocessed dataset.
        """
        VisualTextRegDataset.image_generator = image_generator
        ...
        for sampleno, (source, target) in enumerate(zip(open(source_path, "rt"), open(target_path, "rt")), 1):
            ...
            # VisualTextRegDataset expects the tensor to be flattened
        ...
        dataset = VisualTextRegDataset(source_images, source_sizes,
    ...
    def from_text(
        ...
        return VisualTextRegDataset(source_images, source_sizes,
    ...
    def from_indexed(
        ...
        dataset = VisualTextRegDataset(src_dataset,
