...
#        pygame.display.init()
        ...
        logger.info(f"Image window (default) size {self.window} stride {self.stride}")
    ...
    def get_surface(self, line_text, lang="*", remove_subword=True, window=None):
        ...
        surface_width = max(window, self.estimated_max_char_width * len(line_text))
        ...
        crop_width = max(window, text_rect.width + (self.pad_left + self.pad_right))
        if (crop_width - window) % window != 0:
            ...
            crop_width += window - ((crop_width - self.stride) % window)
        ...
        if crop_width > surface_width:
            ...
            logger.warning(f"{len(line_text)} raw text chars ({old_width // window} slices) too wide, truncating {old_width} -> {crop_width}")
    ...
    def get_image(self, text, lang="*", window=None):
        """
        Returns a single image from a line of text.
        Dimensions: height x width
        """
        return self.get_image_from_surface(self.get_surface(text, window=window))
    ...
    def get_tensor(self, text, window=None):
        """Returns a single tensor for the image.
        Shape: (channels=1 x height x width)
        """        
        image = self.get_image(text, window=window)
        image_tensor = self.transform(image)
        return (image_tensor, window)

    def slice(self, image_tensor_tuple):
        """Slices a tensor according to stride and window.

        image_tensor: Shape (channels, height, width).
        """
        image_tensor, window = image_tensor_tuple
        num_channels, height, width = image_tensor.shape
        
        tensors = []
        # print("IMAGE_TENSOR:", image_tensor.shape)
        for i in range(0, width - window + 1, self.stride):
            # print(f"-> {i}:{i+self.window} of {image_tensor.shape[2]}")
            slice_tensor = image_tensor[:,:,i:i+window]

#            # resize to constant size, self.height x self.window
#            resize = transforms.Resize((self.image_height, self.window))
#            slice_tensor = resize(slice_tensor)

            # or pad to max reg width, LTRB
            pad = transforms.Pad([30-window, 0, 0, 0])
            slice_tensor = pad(slice_tensor)
            
            tensors.append(slice_tensor)

        return torch.stack(tensors)

    def get_tensors(self, text, window=None):
        ...
        window = window if window is not None else self.window
        return self.slice(self.get_tensor(text, window))
