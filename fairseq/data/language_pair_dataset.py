...
def image_collate(
    ...
    src_images_tensor = torch.zeros(len(samples), len(samples[0]['source']), channel, height, width)
    src_tensor_widths = torch.IntTensor(len(samples))
    if 'source_image' in samples[0]:
        for idx, metadata_item in enumerate(samples):
            for word_idx, word_img in enumerate(metadata_item['source_image']):
                #src_images_tensor[idx, word_idx, :, :, :] = word_img
                #src_tensor_widths[idx, word_idx] = metadata_item['source_image_width'][idx][word_idx]
                continue
    ...
    batch = {
        ...
        'image_net_input': {
            ...
            'src_images_width':src_tensor_widths,
        },
        'target': target,
    }       
...
class ImagePairDataset(FairseqDataset):
    ...
    def collater(self, samples):
        ...
        return image_collate(
            samples, pad_idx=self.src_dict.pad(), eos_idx=self.src_dict.eos(),
            channel=self.image_channel, height=self.image_height, 
